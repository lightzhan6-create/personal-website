...
const adminPage = require('./build/pages/admin.js');
...
const DIRS = {
    ...
    locales: path.join(__dirname, '..', 'content', 'config', 'locales'),
...
function loadUnifiedSocialLinks(configPath) {
    ...
    for (const [name, rawValue] of Object.entries(parsed || {})) {
        const value = rawValue && typeof rawValue === 'object'
            ? rawValue
            : { url: rawValue };
        const urlValue = name === 'email'
            ? normalizeMailto(value.url || value.email)
            : String(value.url || '').trim();
        output[`${name}_enabled`] = value.enabled !== false && !!urlValue;
        output[`${name}_url`] = urlValue;
        if (value.icon_url) output[`${name}_icon_url`] = String(value.icon_url).trim();
    }
...
const socialConfig = {
    ...loadConfig(DIRS.control, 'social', 'social.md', SOCIAL_DEFAULTS),
    ...loadUnifiedSocialLinks(path.join(DIRS.content, 'config', 'social-links.json'))
};
...
const tplAdmin = engine.loadTemplate('template_admin.html');
...
const homeProjects = projectsPage.loadProjects(DIRS.projects).slice(0, 3);
const homePhotos = galleryPage.loadGalleryItems(DIRS.gallery).slice(0, 6);
...
indexPage.generateAll({
    ...
    homeProjects,
    homePhotos,
    socialLinksHtml: engine.generateSocialLinks()
});
...
fs.writeFileSync(
    path.join(DIRS.output, 'posts', 'index.html'),
    fs.readFileSync(path.join(DIRS.output, 'all.html'), 'utf-8'),
    'utf-8'
);
searchPage.generate({
    posts: allPosts,
...
if (fs.existsSync(DIRS.public)) copyDir(DIRS.public, DIRS.output);
if (fs.existsSync(DIRS.locales)) copyDir(DIRS.locales, path.join(DIRS.output, 'locales'));
